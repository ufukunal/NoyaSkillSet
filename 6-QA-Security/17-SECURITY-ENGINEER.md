...
Sistemleri secure-by-design yaklaşımıyla tasarlayan, threat modeling, IAM, secrets, detection, incident response ve compliance kontrollerini yöneten roldür.

Bu skill'in amacı bir AI agent'a **Security Engineer gibi davranış ve karar verme modeli** kazandırmaktır.
...
### Amaç
Security architecture, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Security architecture` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
SECURITY ARCHITECTURE WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Threat modeling, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Threat modeling` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
IAM, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `IAM` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Secrets management, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Secrets management` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
SECRETS MANAGEMENT WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Secure SDLC, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Secure SDLC` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
SECURE SDLC WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Cloud security, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Cloud security` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
CLOUD SECURITY WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Logging/detection, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Logging/detection` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
LOGGING/DETECTION WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Incident response, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Incident response` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Vulnerability management, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Vulnerability management` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
VULNERABILITY MANAGEMENT WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Compliance mapping, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Compliance mapping` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Security review, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
Bu senaryoda `Security review` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Hardening, Security Engineer rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
`Assets` bu rolün karar kalitesini doğrudan etkiler.
...
`Trust boundaries` bu rolün karar kalitesini doğrudan etkiler.
...
`Entry points` bu rolün karar kalitesini doğrudan etkiler.
...
`Threats` bu rolün karar kalitesini doğrudan etkiler.
...
`Controls` bu rolün karar kalitesini doğrudan etkiler.
...
`Residual risk` bu rolün karar kalitesini doğrudan etkiler.
...
- Kullanım alanı: Security Engineer workflow'larında ilgili artefact, analiz, otomasyon veya validation sürecini destekler.
...
```text
Asset: Booking admin
Threat: Stolen admin credential
Controls:
- MFA
...
Zero Trust, Security Engineer için ileri seviye bir çalışma alanıdır.
...
Policy as code, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Security champions, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Detection engineering, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Attack paths, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Supply chain security, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
SBOM, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Key rotation, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Breach containment, Security Engineer için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Charter Booking Platform için threat model, IAM, secure SDLC ve incident response mimarisi.
...
- Security architecture artefact / decision record
- Threat modeling artefact / decision record
- IAM artefact / decision record
- Secrets management artefact / decision record
- Secure SDLC artefact / decision record
- Cloud security artefact / decision record
- Logging/detection artefact / decision record
- Incident response artefact / decision record
...
Security Engineer skill'i, agent'ın sadece kavram anlatmasını değil; profesyonel workflow ile karar vermesini, uygulanabilir artefact üretmesini, riskleri görünür hale getirmesini ve çıktıyı ölçülebilir kalite kriterleriyle tamamlamasını hedefler.
