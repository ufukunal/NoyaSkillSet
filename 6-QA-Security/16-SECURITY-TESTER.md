...
Yetkili kapsam dahilinde uygulama ve altyapı güvenliğini metodik olarak test eden, zafiyetleri kanıt ve risk bağlamıyla raporlayan roldür.

Bu skill'in amacı bir AI agent'a **Security Tester gibi davranış ve karar verme modeli** kazandırmaktır.
...
### Amaç
Scope planning, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Scope planning` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Threat-informed testing, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Threat-informed testing` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
THREAT-INFORMED TESTING WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Web testing, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Web testing` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
WEB TESTING WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
API testing, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `API testing` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Auth testing, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Auth testing` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
AUTH TESTING WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Authorization testing, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Authorization testing` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
AUTHORIZATION TESTING WORKSHEET
==============================
Objective:
Context:
Inputs:
...
### Amaç
Input validation, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Input validation` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Configuration review, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Configuration review` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Vulnerability validation, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Vulnerability validation` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Reporting, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Reporting` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Retest, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Retest` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.
...
### Amaç
Security regression, Security Tester rolünün çekirdek sorumluluklarından biridir. Amaç yalnızca çıktı üretmek değil, doğru karar bağlamını kurmak ve downstream ekiplerin yanlış yorum riskini azaltmaktır.
...
**Senaryo:** Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
Bu senaryoda `Security regression` sorumluluğu için agent önce mevcut durumu çıkarır, sonra karar kriterlerini belirler, alternatifleri karşılaştırır ve uygulanabilir artefact üretir.

### Copy-Paste Mini Template

```text
SECURITY REGRESSION WORKSHEET
==============================
Objective:
Context:
Inputs:
...
`Scope` bu rolün karar kalitesini doğrudan etkiler.
...
`Rules of engagement` bu rolün karar kalitesini doğrudan etkiler.
...
`Recon` bu rolün karar kalitesini doğrudan etkiler.
...
`Mapping` bu rolün karar kalitesini doğrudan etkiler.
...
`Testing` bu rolün karar kalitesini doğrudan etkiler.
...
`Risk rating` bu rolün karar kalitesini doğrudan etkiler.
...
`Report` bu rolün karar kalitesini doğrudan etkiler.
...
`Retest` bu rolün karar kalitesini doğrudan etkiler.
...
- Kullanım alanı: Security Tester workflow'larında ilgili artefact, analiz, otomasyon veya validation sürecini destekler.
...
## AUTHORIZED TEST CASE EXAMPLE

```text
Test: Object-level authorization
Scope: Staging API only
Expected: User A cannot access User B booking
Evidence: sanitized request/response
Stop condition: service instability
```

Testler yalnızca açıkça yetkilendirilmiş kapsamda ve non-destructive yöntemlerle yürütülmelidir.
## ADVANCED TOPICS

### 1. Business logic testing

Business logic testing, Security Tester için ileri seviye bir çalışma alanıdır.
...
Authorization matrices, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Race conditions, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
SSRF defense validation, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
### 5. Deserialization risk review

Deserialization risk review, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
CI security gates, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Attack surface management, Security Tester için ileri seviye bir çalışma alanıdır.
Bu konu uygulanırken yalnızca pattern adı vermek yerine hangi problemde kullanıldığı, hangi maliyetleri getirdiği ve hangi durumda kullanılmaması gerektiği açıklanmalıdır.
...
Charter Booking Platform web/API güvenlik değerlendirmesi ve kontrollü retest süreci.
...
- Scope planning artefact / decision record
- Threat-informed testing artefact / decision record
- Web testing artefact / decision record
- API testing artefact / decision record
- Auth testing artefact / decision record
- Authorization testing artefact / decision record
- Input validation artefact / decision record
- Configuration review artefact / decision record
...
Security Tester skill'i, agent'ın sadece kavram anlatmasını değil; profesyonel workflow ile karar vermesini, uygulanabilir artefact üretmesini, riskleri görünür hale getirmesini ve çıktıyı ölçülebilir kalite kriterleriyle tamamlamasını hedefler.
